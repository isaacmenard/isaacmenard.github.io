...
function end(text) {
  ...
  document.getElementsByClassName("end")[0].style.bottom = "0"
  document.getElementById("textEnd").innerHTML = text
}

function sendEnd(pseudo) {
  var xhttp = new XMLHttpRequest();
  xhttp.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      document.getElementById("listScore").innerHTML = this.responseText;
    }
  };
  if (minutes < 10) {
...
function update() {
  if (money < 0) {
    end("Vous avez PERDU :) vous n'avez plus d'argent")
  }
...
function randomBat() {
  ...
  if (listBat.length > 0) {
    ...
    end("Vous avez PERDU :) vous n'avez plus de rayons disponibles")
  } else {
    money -= randomNum(2, 8)
    refreshMoney()
    if (canSpeak == true) {
